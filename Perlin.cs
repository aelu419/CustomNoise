...
public class Perlin
{
    ...
    private int w; //the width of square/cube/...
    ...
    public float Noise(float[] v)
    {
        if (v.Length != d )
        {
            throw new ArgumentException(
                "noisemap's dimension " + d + " is less than the requested vector's dimension" + v.Length
                );
        }

        else
        {
            return Noise(new VecN(v));
        }
    }
